#!/usr/bin/env bash
# Runs the README's example for real, in a throwaway git repository.
#
#   ./docs/demo.sh            print the transcript
#   ./docs/demo.sh --svg OUT  also render the transcript to an animated SVG
#
...
scene() { # scene <extra-resource-line> <extra-node-block>
	cat <<EOF
...
if [ "${1-}" = "--svg" ]; then
	svg_out=${2:?usage: demo.sh --svg OUT.svg}
